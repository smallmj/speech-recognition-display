...
pub fn setup(app: &mut tauri::App) -> Result<(), Box<dyn std::error::Error>> {
    ...
    //    用 app.handle().listen()（Tauri 2 App 本身无 listen 方法）。
    let handle = app.handle().clone();
    let listener = handle.listen(ENGINE_EVENT, move |event| {
...
fn build_tray(
    app: &mut tauri::App,
) -> Result<(TrayIcon, MenuItem, MenuItem), Box<dyn std::error::Error>> {
    ...
    // 默认「识别中」（对齐 spawn_engine_emitter 启动时自动 SessionStarted），
    // 事件流到达后由状态镜像闭包校正文本/可用性。
    let start_i = MenuItem::with_id(app, MENU_START, "▶ 开始识别", false, None::<&str>)?;
    let stop_i = MenuItem::with_id(app, MENU_STOP, "⏸ 停止识别并生成纪要", true, None::<&str>)?;
