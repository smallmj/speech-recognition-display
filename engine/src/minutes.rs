...
pub fn chunk_for_summarize(segments: &[Segment], max_chars_per_batch: usize) -> Vec<Vec<String>> {
    ...
    for text in texts {
        ...
        let fits_current = !closed_overlong
            && batches.last().is_some_and(|batch| !batch.is_empty())
...
mod tests {
    ...
    // ---- 验收：默认分批参数不超 2000 token 预算 ----

    #[test]
    fn default_batch_stays_within_token_budget() {
        assert!(
            BATCH_MAX_CHARS + ROLLING_CONTEXT_CHARS <= MAX_TOKENS,
            "正文 + 滚动上文应远低于 2000 token 预算"
        );
        let segments: Vec<Segment> = (0..2).map(|i| seg(i, &"字".repeat(300))).collect();
        for batch in chunk_for_summarize(&segments, BATCH_MAX_CHARS) {
            let total_chars: usize = batch.iter().map(|piece| piece.chars().count()).sum();
            assert!(
                total_chars <= MAX_TOKENS,
                "单批总字符不应超 token 预算: {total_chars}"
            );
        }
    }
    ...
    fn rolling_context_appears_at_start_of_second_and_later_batches() {
        ...
        assert!(
            rc2.starts_with(ROLLING_CONTEXT_MARKER),
            "滚动上文带标记: {rc2}"
        );
        assert!(
            rc2.contains(&"甲".repeat(80)),
            "第 2 批滚动上文 = 上一批正文末尾: {rc2}"
        );
        ...
        assert!(
            rc3.contains(&"乙".repeat(80)),
            "第 3 批滚动上文 = 第 2 批正文末尾: {rc3}"
        );
    ...
    fn rolling_context_is_capped_at_constant_chars() {
        ...
        let segments = vec![
            seg(0, &format!("{}{}", "头".repeat(200), "尾".repeat(100))),
            seg(1, "二"),
        ];
        ...
        assert_eq!(
            tail.chars().count(),
            ROLLING_CONTEXT_CHARS,
            "滚动上文长度封顶 100 字"
        );
        assert_eq!(
            tail,
            &"尾".repeat(100),
            "取的是上一批末尾（尾×100），不是开头（头）"
        );
    }
    ...
    fn empty_input_yields_no_batches() {
        assert!(
            chunk_for_summarize(&[], BATCH_MAX_CHARS).is_empty(),
            "无片段 → 空批"
        );
    }
    ...
    fn overlong_single_segment_gets_own_batch_not_truncated() {
        ...
        assert!(
            batch_content_chars(&batches[0]) > 100,
            "超长批正文超上限（有意为之）"
        );

        // 后续片段在新批（[0] 为该批的滚动上文，正文从 [1] 开始），不受超长批影响
        assert_eq!(
            batch_content_chars(&batches[1]),
            2,
            "第 2 批正文只有「短句」2 字"
        );
    ...
    fn summarize_minutes_aggregates_partials_in_window_order() {
        ...
        assert!(
            minutes.starts_with("要点："),
            "汇总输出为要点开头: {minutes}"
        );
        assert!(
            minutes.ends_with("（共 4 段）"),
            "汇总收到 4 段部分纪要（顺序即收集顺序）: {minutes}"
        );
        // 批 1 只有片段 0 一个元素 → 其部分纪要形状为「（共 1 段）」，且在汇总开头
        assert!(
            minutes.contains(&format!("{}（共 1 段）", "甲".repeat(60))),
            "批 1 部分纪要按序在前: {minutes}"
        );
        // 批 2+ 含滚动上文元素 → 其部分纪要形状为「（共 2 段）」
        assert!(
            minutes.contains("（共 2 段）"),
            "批 2+ 部分纪要含滚动上文元素: {minutes}"
        );
    }
    ...
    fn summarize_minutes_empty_session_returns_placeholder() {
        assert_eq!(
            summarize_minutes(&MockLlmPort, &[], BATCH_MAX_CHARS),
            "（无内容）"
        );
        let blank = vec![seg_raw(0, "  ")];
        assert_eq!(
            summarize_minutes(&MockLlmPort, &blank, BATCH_MAX_CHARS),
            "（无内容）"
        );
    }
